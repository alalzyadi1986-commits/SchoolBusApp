...
export default function SchoolScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    onValue(ref(db, `schools/${schoolId}`), (snap) => {
      ...
      if (data) {
        ...
        if (data.location) setSchoolLocation(data.location);
      }
    });
  ...
  const saveSchoolLocation = async (coordinate) => {
    try {
      ...
      setSchoolLocation(coordinate);
      setShowMapPicker(false);
      Alert.alert('تم', 'تم حفظ موقع المدرسة بنجاح ✅');
    } catch (error) {
      Alert.alert('خطأ', 'فشل حفظ الموقع');
    }
  };
      ...
      <View style={styles.tabBar}>
        <View style={styles.tabGrid}>
          {[
            ...
            { id: 'location', label: 'موقع المدرسة' }
          ].map(tab => (
            <TouchableOpacity
              key={tab.id}
              style={[styles.tabGridItem, activeTab === tab.id && styles.activeTabGrid]}
              onPress={() => { setActiveTab(tab.id); setFormData({}); setEditingId(null); }}
            >
      ...
      <ScrollView style={styles.content}>
        ...
        {activeTab === 'location' && (
          <View style={styles.formCard}>
            <Text style={styles.formTitle}>📍 موقع المدرسة</Text>
            {schoolLocation ? (
              <Text style={styles.locationText}>
                ✅ تم تحديد الموقع: {schoolLocation.latitude.toFixed(4)}, {schoolLocation.longitude.toFixed(4)}
              </Text>
            ) : (
              <Text style={styles.locationText}>⚠️ لم يتم تحديد موقع المدرسة بعد</Text>
            )}
            <TouchableOpacity style={styles.saveBtn} onPress={() => setShowMapPicker(true)}>
              <Text style={styles.saveBtnText}>📍 تحديد موقع المدرسة على الخريطة</Text>
            </TouchableOpacity>
            {showMapPicker && (
              <View style={{ marginTop: 15 }}>
                <Text style={styles.inputLabel}>اضغط على الخريطة لتحديد موقع المدرسة:</Text>
                <MapView
                  style={{ width: '100%', height: 300, borderRadius: 12, marginTop: 10 }}
                  initialRegion={{
                    latitude: schoolLocation?.latitude || 31.9454,
                    longitude: schoolLocation?.longitude || 35.9284,
                    latitudeDelta: 0.05,
                    longitudeDelta: 0.05
                  }}
                  onPress={(e) => saveSchoolLocation(e.nativeEvent.coordinate)}
                >
                  {schoolLocation && (
                    <Marker coordinate={schoolLocation} title="موقع المدرسة" pinColor="blue" />
                  )}
                </MapView>
                <TouchableOpacity style={[styles.saveBtn, { backgroundColor: '#94A3B8', marginTop: 10 }]} onPress={() => setShowMapPicker(false)}>
                  <Text style={styles.saveBtnText}>إغلاق الخريطة</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
        ...
        {activeTab === 'drivers' && showDriverForm && (
          <View style={styles.formCard}>
            ...
            <View style={styles.inputWrapper}>
              <Text style={styles.inputLabel}>السرعة القصوى:</Text>
              ...
              <View style={styles.speedPickerModal}>
                ...
                <ScrollView style={{ maxHeight: 200 }} nestedScrollEnabled={true}>
                  {speedOptions.map((speed, idx) => (
                    ...
                    >
                      <Text style={[styles.speedOptionText, formData.max_speed === speed && styles.speedOptionTextActive]}>
                        {speed === 'بدون تحديد' ? 'بدون تحديد' : `${speed} كم/س`}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
        ...
        {activeTab === 'parents' && showParentForm && (
          <View style={styles.formCard}>
            ...
            {renderInput('كلمة المرور', 'password')}
            {renderInput('رقم الهاتف', 'phone', true)}
            {renderInput('اسم العائلة', 'family_name')}
            <TouchableOpacity style={styles.saveBtn} onPress={() => handleAction('save')}>
              <Text style={styles.saveBtnText}>{editingId ? 'تحديث البيانات' : 'حفظ البيانات'}</Text>
            </TouchableOpacity>
            {editingId && (
        ...
        {activeTab === 'students' && (
          <View style={styles.filterSection}>
            ...
            <Text style={styles.filterLabel}>تصفية حسب الصف:</Text>
          ...
          <View style={styles.listContainer}>
            <View style={styles.listHeader}>
              <Text style={styles.listTitle}>
                {activeTab === 'drivers' ? `السائقون (${drivers.length})` :
                 activeTab === 'staff' ? `المرافقون (${staff.length})` :
                 activeTab === 'parents' ? `أولياء الأمور (${parents.length})` :
                 activeTab === 'students' ? `الطلاب (${filteredStudents.length})` :
                 activeTab === 'reports' ? 'التقارير' :
                 activeTab === 'managers' ? `المديرون (${managers.length})` : 'الطوارئ'}
              </Text>
            </View>
            ...
            {activeTab === 'reports' && (
              <View style={styles.reportFilter}>
                <ScrollView horizontal style={styles.chipScroll}>
                  ...
                  {drivers.map(d => (
                    <TouchableOpacity key={d.id} style={[styles.miniChip, selectedDriverReport === d.username && styles.miniChipActive]} onPress={() => setSelectedDriverReport(selectedDriverReport === d.username ? 'الكل' : d.username)}>
            ...
            {(activeTab === 'reports' ? filteredReports :
              ...
              activeTab === 'managers' ? managers : emergencies).map((item) => (
              <View key={item.id} style={styles.card}>
                <View style={styles.cardInfo}>
                  {activeTab === 'drivers' ? (
                    <>
                      <Text style={styles.cardName}>{item.name}</Text>
                      <Text style={styles.cardSub}>باص: {item.bus_number} | سرعة: {item.max_speed || 80} كم/س</Text>
                      <Text style={styles.cardSub}>👤 {item.username} | 🔑 {item.password}</Text>
                    </>
                  ) : activeTab === 'reports' ? (
                    <>
                    ...
                    <>
                      <Text style={styles.cardName}>🆘 {item.senderName}</Text>
                      <Text style={styles.cardSub}>{item.message}</Text>
                      <Text style={styles.cardDate}>{new Date(item.timestamp).toLocaleString('ar-EG')}</Text>
                    </>
                  ) : (
                    <>
                      ...
                      <Text style={styles.cardSub}>
                        {activeTab === 'students' ? `الصف: ${item.class} - ${item.section}` : `📞 ${item.phone} | 🔑 ${item.password}`}
                      </Text>
                    </>
                  )}
                </View>
...
const styles = StyleSheet.create({
  ...
  content: { flex: 1 },
  addBtn: { backgroundColor: '#3B82F6', padding: 15, margin: 15, borderRadius: 12, alignItems: 'center', elevation: 3 },
  ...
  editBtn: { paddingVertical: 8, paddingHorizontal: 15, borderRadius: 8, backgroundColor: '#DBEAFE', marginLeft: 10 },
  ...
  locationText: { fontSize: 14, color: '#475569', textAlign: 'right', marginBottom: 15 },
  speedPickerBtn: { backgroundColor: '#F8FAFC', padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#E2E8F0' },
  speedDisplayText: { fontSize: 15, color: '#1F2937' },
  speedPickerModal: { backgroundColor: '#F8FAFC', borderRadius: 12, padding: 15, marginTop: 10, borderWidth: 1, borderColor: '#E2E8F0' },
  speedPickerTitle: { fontSize: 14, fontWeight: 'bold', textAlign: 'right', marginBottom: 10 },
  speedOption: { padding: 12, borderRadius: 8, marginBottom: 5, backgroundColor: '#FFF' },
  speedOptionActive: { backgroundColor: '#3B82F6' },
  speedOptionText: { fontSize: 14, textAlign: 'right', color: '#1E293B' },
  speedOptionTextActive: { color: '#FFF', fontWeight: 'bold' },
  speedPickerClose: { backgroundColor: '#94A3B8', padding: 10, borderRadius: 8, alignItems: 'center', marginTop: 10 },
