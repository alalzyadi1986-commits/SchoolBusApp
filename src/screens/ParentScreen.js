...
export default function ParentScreen() {
  ...
  useEffect(() => {
    ...
    // صلاحية الموقع الحالي لولي الأمر
    (async () => {
      try {
  ...
  // بيانات السائق والباص وتتبع الموقع
  useEffect(() => {
        ...
        if (staffData) {
          setStaffInfo(
            Object.values(staffData).find(
              (s) => s.driverUsername === driverId || s.driver_id === driverId
            )
          );
        }
      }
    ...
    const unsubBus = onValue(
      ...
      (snap) => {
        ...
        if (busData && busData.isActive && busData.latitude && busData.longitude) {
          ...
          if (isNaN(newLoc.latitude) || isNaN(newLoc.longitude)) return;

          if (!busLocation) {
            setBusLocation(newLoc);
            setAnimatedBusLocation(newLoc);
          } else {
            // نظام التتبع الذكي: تحريك انسيابي بين النقاط (كل 10 ثوانٍ)
            animateBus(animatedBusLocation || busLocation, newLoc);
            setBusLocation(newLoc);
          }

          // تصحيح منطق التنبيهات:
          // نفترض متوسط سرعة الباص 30 كم/س (أي 0.5 كم في الدقيقة)
          // المسافة المطلوبة للتنبيه = alertMinutes * 0.5 كم
          if (myLocation) {
            ...
            const alertThreshold = alertMinutes * 0.5; // كم

            if (dist < alertThreshold && !notified && studentInfo?.status !== 'absent_today') {
              Alert.alert(
                '🔔 تنبيه وصول الباص 🚌',
                `الباص على بعد حوالي ${dist.toFixed(1)} كم من موقعك وسيقوم بالوصول قريباً.`
              );
              setNotified(true);
            } else if (dist > alertThreshold + 0.5) {
              // إعادة تفعيل التنبيه إذا ابتعد الباص (للحالات التي يعود فيها الباص لنفس المنطقة)
              setNotified(false);
            }
          }
        } else {
  ...
  const animateBus = (start, end) => {
    let startTime = null;
    const duration = 10000; // متوافق مع وتيرة تحديث السائق (10 ثوانٍ)
  ...
  const reportAbsence = () => {
    ...
    Alert.alert(
      ...
      isAbsent ? 'هل تريد إلغاء الغياب؟' : 'هل تريد إبلاغ السائق بغياب الطالب اليوم؟',
      [
        { text: 'إلغاء', style: 'cancel' },
        {
          text: 'تأكيد',
          onPress: async () => {
            ...
            await set(push(reportRef), {
              type: 'absence',
              timestamp: new Date().toISOString(),
              studentId: studentInfo.id,
              studentName: studentInfo.name,
              parentUsername: user.username,
              action: isAbsent ? 'cancelled' : 'reported'
            });

            Alert.alert('تم إبلاغ المدرسة والسائق بنجاح');
          },
        },
      ]
    );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ alignItems: 'flex-end' }}>
          ...
          <Text style={styles.parentName}>عائلة {user?.name || 'المحترم'}</Text>
        </View>
      </View>
      ...
      <View style={styles.infoCard}>
        <View style={styles.studentHeader}>
          ...
          <View style={{ alignItems: 'flex-end' }}>
            <Text style={styles.studentName}>{studentInfo?.name || 'طالب غير معروف'}</Text>
      ...
      <View style={styles.settingsCard}>
        <Text style={styles.settingsTitle}>تنبيه القرب (قبل الوصول بـ):</Text>
      ...
      >
        ...
        {animatedBusLocation && (
          <Marker coordinate={animatedBusLocation} title="الباص">
...
const styles = StyleSheet.create({
  ...
  infoCard: { margin: 15, padding: 15, backgroundColor: '#FFF', borderRadius: 15, elevation: 3 },
  ...
  studentStatus: { fontSize: 14, color: '#64748B', marginTop: 4 },
  absenceBtn: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: '#EF4444' },
  absenceBtnActive: { backgroundColor: '#EF4444' },
  absenceBtnText: { color: '#EF4444', fontWeight: 'bold', fontSize: 12 },
  absenceBtnTextActive: { color: '#FFF' },
  contactRow: { flexDirection: 'row', justifyContent: 'space-between' },
  contactBtn: { flex: 0.48, backgroundColor: '#3B82F6', padding: 10, borderRadius: 10, alignItems: 'center' },
  contactBtnText: { color: '#FFF', fontSize: 12, fontWeight: 'bold' },
  settingsCard: { marginHorizontal: 15, marginBottom: 15, padding: 15, backgroundColor: '#FFF', borderRadius: 15, elevation: 2 },
  settingsTitle: { fontSize: 14, fontWeight: 'bold', color: '#1E293B', marginBottom: 10, textAlign: 'right' },
  optionsRow: { flexDirection: 'row', justifyContent: 'space-around' },
  optBtn: { paddingHorizontal: 15, paddingVertical: 8, borderRadius: 10, backgroundColor: '#F1F5F9' },
  optBtnActive: { backgroundColor: '#3B82F6' },
  optText: { color: '#64748B', fontWeight: 'bold' },
  optTextActive: { color: '#FFF' },
  map: { flex: 1, width: '100%', borderRadius: 15, margin: 15, overflow: 'hidden' },
  busMarker: { backgroundColor: '#FFF', padding: 5, borderRadius: 20, elevation: 5, borderWidth: 1, borderColor: '#3B82F6' },
});
